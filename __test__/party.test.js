...
beforeAll(async () => {
    ...
    let user1 = await User.findOne({ where: { role: `Admin` } });
    let user2 = await User.findOne({ where: { role: `Staff` } });
...
describe('Party', () => {
    describe('/party', () => {
        describe('Success', () => {
            ...
            test('should success delete a party', async () => {
                let { body, status } = await request(app)
                    .delete('/party/2')
                    .set('Authorization', `Bearer ` + access_token1)
                expect(status).toBe(200)
                expect(body).toHaveProperty('message', `Party with id 2 has been deleted`)
            });
        })

        describe('Failed', () => {
            test('should fail show party because invalid token', async () => {
                let { body, status } = await request(app)
                    .delete('/party/2')
                    .set('Authorization', `Bearer` + access_token2)
            ...
            test('should fail created team when team is full', async () => {
                let { body, status } = await request(app)
                    .post('/party/1/team')
                    .set('Authorization', `Bearer ` + access_token1)
                    .send({
                ...
                expect(status).toBe(400)
                expect(body).toHaveProperty('message', "Your team can only have 4 Character")
            });
